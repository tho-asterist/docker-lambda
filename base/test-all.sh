...
docker run --rm -v "$PWD":/var/task mlupin/docker-lambda:nodejs8.10
docker run --rm -v "$PWD":/var/task mlupin/docker-lambda:nodejs10.x index.handler
...
cd ${EXAMPLES_DIR}/python
docker run --rm -it mlupin/docker-lambda:build-python2.7 pip install marisa-trie
docker run --rm -it mlupin/docker-lambda:build-python3.6 pip install marisa-trie
docker run --rm -it mlupin/docker-lambda:build-python3.7 pip install marisa-trie
...
docker run --rm -v "$PWD":/app -w /app mlupin/docker-lambda:build-java8 gradle build
docker run --rm -v "$PWD/build/docker":/var/task mlupin/docker-lambda:java8 org.lambci.lambda.ExampleHandler '{"some": "event"}'
docker run --rm -v "$PWD/build/docker":/var/task mlupin/docker-lambda:java8.al2 org.lambci.lambda.ExampleHandler '{"some": "event"}'
docker run --rm -v "$PWD/build/docker":/var/task mlupin/docker-lambda:java11 org.lambci.lambda.ExampleHandler '{"some": "event"}'
...
docker run --rm -v "$PWD":/var/task mlupin/docker-lambda:build-dotnetcore2.1 dotnet publish -c Release -o pub
docker run --rm -v "$PWD"/pub:/var/task mlupin/docker-lambda:dotnetcore2.1 test::test.Function::FunctionHandler '{"some": "event"}'
...
docker run --rm -v "$PWD":/var/task mlupin/docker-lambda:provided handler '{"some": "event"}'
...
#   mlupin/docker-lambda:ruby2.5 lambda_function.lambda_handler
